...
- `plugin.json` also declared `"agents": ["./agents/researcher.md"]`, and `claude plugin details devio` counted `Agents (0)` because of it. Dropping the key — `agents/` is picked up on its own — registers the `researcher` sub-agent. Same trap as the `hooks` key.
